...
function dostup_org() {
    ...
    global $id_user;
    if (!($id_user == "")) {
        $ok_tip = mysql_query("SELECT * FROM org where id_org = $id_user and god = 2015");
...
function registr() {
    ...
    while ($t_page_s = mysql_fetch_array($ok_page_s)) {
     ...
     <a href = \"$site/registr.php\" class=\"underline\"><strong><font color = \"#FE0505\">Подтвердиться</font></strong></a><br>";
                ...
                echo "<!-- <br><a href = \"$site/registr.php\" class=\"underline\"><strong><font color = \"#FE0505\">Зарегистрироваться</font></strong></a><br> -->"; //убрать скрытие на следующий сибкон
            }
        } else
        if ($regpodpos == "3") {
            if ($status == "1") {
                ...
                if ($poselen == "") {
                    		...
                    		<a href = \"$site/room.php?id=665\" class=\"underline\"><strong><font color = \"#FE0505\">Дневное посещение</font></strong></a>
							"; //убрать скрытие на следующий сибкон
                } else {
		...
		// убрать коммент на списке регистрации и поселения после 1го ноября.
 ...
 //  ";
        ...
        $ok_reg3 = mysql_query("select * from room_us  WHERE `god` = '$god_site' and `id_command` != 665");
        $itog_reg13 = mysql_num_rows($ok_reg3);
        $itog_reg123 = $itog_reg1 + $itog_reg12;
		$ok_reg4 = mysql_query("select * from room_us  WHERE `god` = '$god_site' and `id_command` = 665");
