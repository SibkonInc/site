...
include_once ('header.php');
 ...
 <div id="center_wrap">
	<div id="one_column">
	<center><b>Деньги на базе</b>
	
	<table border=1><tr><td>Участник</td><td>Должен оплатить</td><td>Питание</td><td>Оплатил</td><td>Тип оплаты</td><td>Когда оплатил</td><td>Комментарий</td></tr>
	<?php
...
if($sum_pit == "1500" and strtotime($str) <= strtotime($date_opl) and strtotime($str) > strtotime($null_date)) $sum_pit = $chet + $pyat + $subb + $vos;}}else{$sum_pit = "0";}}																
			...
			if($predoplata == 1 and $summa_opl > 0)
				$krovat++;
				else if($summa_opl > 0 and $predoplata == 0)
				$krovat_baza++;
                   	if ($tip_opl == 0){
					$orgvznos;									
					$poln=$summ_k;
											...
											}else if ($tip_opl == 3) {$poln=0;}else if ($tip_opl == 5) {$orgvznos = 750;$poln=$st_kro + $orgvznos;}
						...
						if ($tip_opl == 0){
							...
							$poln=$summ_l;							
										...
										$poln=$orgvznos;
											}else if ($tip_opl == 3) {$poln=0;}else if ($tip_opl == 5) {$orgvznos = 750;$poln=$st_lux + $orgvznos;}
						...
						if ($tip_opl == 0){							
							...
							$poln=$st_spa;
											...
											}else if ($tip_opl == 3) {$poln=0;}else if ($tip_opl == 5) {$orgvznos = 750;$poln=$st_spa + $orgvznos;}
							}else if ($tip_room == 4) {
...
?>
	...
	<div id="two_column"><center>
		<table border=1 width=342px>
		...
		<tr> <td colspan=3><b>Всего на базе: <?php echo"$baza человек";?></b></td></tr>
		<?php $vsego = $spaln + $krovat + $lyks; $dnev_all = $dnev + $dnev_baza?>
		<tr> <td colspan=3>Всего по сайту без дневников: <?php echo"$vsego человек";?></td></tr>
		<tr><td>Дневное</td><td><?php echo"$dnev_all человек";?></td><td><?php echo"$dnev человек";?></td></tr>
		<tr><td>Дневники оплатили:</td><td><?php echo"$dnev_money .р";?></td><td></td></tr>
		
		<?php
		$ok_page = "select * from stat_baza where god = $god_site and sanvznos = 1";
		$result = mysql_query($ok_page);
		$sdalo_san = mysql_num_rows($result);
		$sql = "SELECT SUM(`summa_sanvznos`) as `sum` FROM `stat_baza`where god = $god_site";
		$result1 = mysql_query($sql);
		$row = mysql_fetch_assoc($result1);
		$var = $row['sum'];//Тут хранится сумма
		$ok_page = "select * from stat_baza where god = $god_site and proverka = 1";
		$result = mysql_query($ok_page);
		$priehalo = mysql_num_rows($result);
		?>
		<tr><td>Сан. Взнос:</td><td><?php echo"сдало $sdalo_san человек </td><td>$var р.</td></tr>";?>
		<tr><td><b>Всего приехало:</b></td><td><?php echo"$priehalo человек </td><td></td></tr>";?>
		</table>
		<table border=1 width=342px>
		...
		<tr><td>Денег за еду уйдет:</td><td colspan=4><?php echo"$all_pit_money .р";?></td></tr>
		</table>
	<table border=1 width=342px>
		<tr>
			<td colspan=3>
		...
		<!--<table  border=1  width=342px>
				...
				<tr>
			<td colspan=3>
			<b>После предоплаты и до базы</b>
			</td>
			</tr>
		<tr> <td colspan=3> Всего:
		<?php
