...
<span class=\"ncc\"><a href=\"$site/registr_off.php?id=1&id_us=$id_us\">Снять регистрацию на $name_site</a></span> 
<b class=\"nc\"><b class=\"nc2\"><b></b></b><b class=\"nc1\"><b></b></b></b>
            ...
            if ($id == "1")
            {
               ...
               //выясняем поселение, кто главынй и выселяем с отправлением уведомления главному
                     ...
                     while ($registr_sr = mysql_fetch_array($registrr))
                     {
                        ...
                        if ($id_us_room == "$id_us")
                        {
                           ...
                           while ($registr_srg = mysql_fetch_array($registrrg))
                           {
                              ...
                              $tip = "Отказ от участия";
                              $text = "Пользователь $name_user_off отказался от проживания в $name_room в качестве главного";
                              ad_log($text);
                              pager_post($room_glav, $id_us, $text, $tip, $nit);
                              echo "<li>Сообщение ответственному $id_us_glav_name о снятии вас с должности Главного в $name_room отправлено</li>";
                           }
                           mysql_query("DELETE FROM room_us WHERE id = $id_room1");
                           echo "<li><b>Снято ваше проживание в $name_room</b></li>";
                        }
                        else
                        {
                           ...
                           $name_room = name_room($id_room);
                           $tip = "Отказ от участия";
                           $textr = "Пользователь $name_user_off отказался от проживания в $name_room ";
                           ad_log($textr);
                           pager_post($id_us_room, $id_us, $text, $tip, $nit);
                           echo "<li>Сообщение главному $id_r_glav_name о снятии вас с проживания в $name_room отправлено</li>";
               ...
               else
               {
                  ...
                  $mess_log = "Снял регистрацию подопечного $name_podop с $name_site $god_site";
               }
               ad_log($mess_log);
               mysql_query("DELETE FROM registr WHERE   god = '$god_site' and id_us = '$id_us'");
