...
case 'add_tip':if ($this->access['access_admin'] == "1") {$this->add_tip($url['id']);} else {$this->index();} break;
    ...
    function admin($message=false) {
        ...
        $data['file'] = "modules/partners/admin/admin_part_html.php";
    ...
    function admin_tip($message=false) {
        ...
        $data['file'] = "modules/partners/html/admin_tip_html.php";
        $this->load_theme($data);
    }
    ...
    function edit_tip($id,$message=false) {
        ...
        if (empty($_POST)) {
            ...
            $data['file'] = "modules/partners/html/admin_edit_tip_html.php";
            $this->load_theme($data);
    ...
    function add_tip($id,$message=false) {
        ...
        if (empty($_POST)) {
            ...
            $data['message'] = $message;
            $data['access'] = $this->access;
            $data['file'] = "modules/partners/html/admin_add_tip_html.php";
            $this->load_theme($data);
    ...
    function index($message=false) {
        ...
        if ($url['action'] == "tip") {
            $data['part_lang'] = $this->sql_query_one("m_part_lang", "where id_lang='" . $this->setting['id_lang'] . "'
               ");
            $data['part_data'] = $this->sql_query("m_partners", "
           join m_part_text on m_partners.part_id = m_part_text.part_id
           
           where m_part_text.id_lang='" . $this->setting['id_lang'] . "' 
               and m_partners.part_tip ='" . $url['id'] . "'  order by part_nomer");
        } else {
            $data['part_lang'] = $this->sql_query_one("m_part_lang", "where id_lang='" . $this->setting['id_lang'] . "'");
            $data['part_data'] = $this->sql_query("m_partners", "
                join m_part_text on m_partners.part_id = m_part_text.part_id
        ...
        $data['file'] = "modules/partners/html/index_part_html.php";
        $data['access'] = $this->access;
        $this->load_theme($data);
    }
...
//данные для титла
        ...
        $data['file'] = "modules/partners/html/add_part_html.php";
        $data['access'] = $this->access;
        $this->load_theme($data);
    }
    ...
    function add_post() {
        ...
        if (empty($_POST)) {
            ...
            $post = array();
            $message = array();
           if (copy($_FILES["part_logo"]["tmp_name"],"img/partners/" . $_FILES["part_logo"]["name"])) {
            ...
            //     var_dump($post);
            $id_post = $this->insert("m_partners", $post);
            if ($id_post == false) {
...
//вызываем просмотр без сообщений
    ...
    function view($id, $message=false) {
        ...
        $data['message'] = $message;
        $data['access'] = $this->access;
        $part_data = $this->sql_query_one("m_partners", "join m_part_text on m_partners.part_id = m_part_text.part_id join m_part_tip_lang on m_partners.part_tip = m_part_tip_lang.id_part_tip where m_part_text.id_lang='" . $this->setting['id_lang'] . "' and m_partners.part_id='" . $id . "'");
        ...
        $data['file'] = "modules/partners/html/view_part_html.php";
        $this->load_theme($data);
    }
...
//название для самого модуля
        $data['part_tip_data'] = $this->sql_query("m_part_tip", "join m_part_tip_lang on m_part_tip.id_part_tip = m_part_tip_lang.id_part_tip where m_part_tip_lang.id_lang='" . $this->setting['id_lang'] . "'");
        $part_data = $this->sql_query_one("m_partners", "where m_partners.part_id='" . $id . "'");
        $data['part_data'] = $part_data;
        $data['page'] = $this->get_name_module_page($this->identifcator) . " | Редактирование";
        $data['file'] = "modules/partners/html/edit_part_html.php";
        $data['access'] = $this->access;
        $this->load_theme($data);
    }
    ...
    function edit_post($id) {
        ...
        if (empty($_POST)) {
            ...
            if (!($_FILES["part_log"]["tmp_name"] == "")) {
                if (copy($_FILES["part_log"]["tmp_name"],"img/partners/" . $_FILES["part_log"]["name"])) {
                    $message['fil'] = "Файл успешно загружен <br>";
                    $message['fil'].="Характеристики файла: <br>";
                    $message['fil'].="Имя файла: ";
            ...
            isset($_POST['part_tip']) ? $post['part_tip'] = $_POST['part_tip'] : false;
            isset($_POST['part_nomer']) ? $post['part_nomer'] = $_POST['part_nomer'] : false;
            $id_post = $this->sql_edit("m_partners", $post, "where part_id='" . $id . "'");
            //обрабатываем
            if ($id_post == false) {
    ...
    function del($id) {
        global $setting;
        include_once 'modules/partners/html/del_part.php';
    }

    function del_post($id) {
        global $setting;
        $message = array();
        $o = $this->sql_del("m_partners", "where part_id = '" . $id . "'");
