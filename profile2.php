...
<form action="room_opros_save.php" id='opros_form' method='post' >
   ...
   <input type="radio" name='pitanie' id='pitanie_3' value='3' <?php if(($res == 3) ||  ($opros->pitanie == "3")) { ?>checked<?php } ?>/><label for="pitanie_3" checked>Питаюсь по предоплате </label><br/>
...
$('#pitanie_1, #pitanie_2, #pitanie_3').click(function () {
check_pitanie();
});
check_pitanie();
...
<input type=submit value="<?php if ($res == "3") { echo "Продолжить";} else {echo "Сохранить";} ?> "/> 
</form>
</div>
<?php
}
...
<tr><td>Город: </td><td><a href = \"$site/users.php?g=$gorod_us\">$gorod_us</a></td></tr>";
		...
		if ($id_u == $id_user) {
			...
			echo "<h3><a href=\"#\">Питание:</a></h3><div>"; opros2(); echo"</div>";				
		}
        if ($id_u == $id_user or $adm == 1 or $master == $id_user or $id_user=="539") {
            echo "
<p>
