...
<tr class=top><th style="background:#dadada;">Чт</th><th style="background:#decbc8;">Пт</th><th style="background:#e4e8c9;">Сб</th><th style="background:#dadada;">Вс</th></tr>
<?php
while ($t_opros = mysql_fetch_array($ok_page))
{
        ...
        echo "<tr>";
?>
		<td align=center style="background:#dadada;"><?php if($opros->pitanie != "1" && in_array("1", $opros->pitanie_choice)) { ?>+<?php } ?></td>

		<td align=center style="background:#decbc8;"><?php if($opros->pitanie != "1" && in_array("2", $opros->pitanie_choice)) { ?>+<?php } ?></td>

		<td align=center style="background:#e4e8c9;"><?php if($opros->pitanie != "1" && in_array("5", $opros->pitanie_choice)) { ?>+<?php } ?></td>

		<td align=center style="background:#dadada;"><?php if($opros->pitanie != "1" && in_array("8", $opros->pitanie_choice)) { ?>+<?php } ?></td>
</tr>
<?php 
}
echo "</table>";
...
if (!empty($t_opros["data"])) {
	...
	<?php if ($opros->pitanie == "3") { echo "<Strong>Буду питаться по предоплате</Strong>";  }?>
...
<div style='float:left; width: 100px'>
		...
		<?php if(in_array("1", $opros->pitanie_choice)) { ?>Питаюсь!<?php } ?><br />
</div>
<div style='float:left; width: 100px'>
		<strong>Пятница</strong><br />
		<?php if(in_array("2", $opros->pitanie_choice)) { ?>Питаюсь!<?php } ?><br />
</div>
<div style='float:left; width: 100px'>
		<strong>Суббота</strong><br />
		<?php if(in_array("5", $opros->pitanie_choice)) { ?>Питаюсь!<?php } ?><br />
</div>
<div style='float:left; width: 100px'>
		<strong>Воскресенье</strong><br />
		<?php if(in_array("8", $opros->pitanie_choice)) { ?>Питаюсь!<?php } ?><br />
</div>
	</div> 
<?php } ?>
<Br style="clear:both;"/>
...
<div style='float:left; width: 100px'>
		<strong>Четверг</strong><br />
		<input type='checkbox' name='pitanie_choice[]' value='1' id='pitanie_choice_1' <?php if(($res == 2) || in_array("1", $opros->pitanie_choice)) { ?>checked<?php } ?>/><label for='pitanie_choice_1'> Питаюсь!</label><br />
</div>
<div style='float:left; width: 100px'>
		<strong>Пятница</strong><br />
		<input type='checkbox' name='pitanie_choice[]' value='2' id='pitanie_choice_2' <?php if(($res == 2) || in_array("2", $opros->pitanie_choice)) { ?>checked<?php } ?>/><label for='pitanie_choice_2'> Питаюсь!</label><br />
</div>
<div style='float:left; width: 100px'>
		<strong>Суббота</strong><br />
		<input type='checkbox' name='pitanie_choice[]' value='5' id='pitanie_choice_5' <?php if(($res == 2) || in_array("5", $opros->pitanie_choice)) { ?>checked<?php } ?>/><label for='pitanie_choice_5'> Питаюсь!</label><br />

</div>
<div style='float:left; width: 100px'>
		<strong>Воскресенье</strong><br />
		<input type='checkbox' name='pitanie_choice[]' value='8' id='pitanie_choice_8' <?php if(($res == 2) || in_array("8", $opros->pitanie_choice)) { ?>checked<?php } ?>/><label for='pitanie_choice_8'> Питаюсь!</label><br />
</div>
	</div> 
</fieldset>
...
if (!empty($t_opros["data"])) {
	$opros = @json_decode($t_opros["data"]);
	if (!empty($opros)) {
	if ($opros->pitanie == "3") {$sum_pit=$predop; echo "<br>Цена питания: $sum_pit р." ;  } else
		if ($opros->pitanie == "1") { echo "<br>Не питаюсь";  } else
			if ($opros->pitanie == "2") { echo "<br>Цена питания:";
			 ...
			 echo "$sum_pit р.";
                    ...
                    echo "<p>Тип поселения: Кровати<br>";
						if ($tip_opl == 0){
							...
							echo "Полная оплата, осталось внести: "; $poln=$summ_k - $summa_opl + $sum_pit; echo "$poln.р";
							} else {if ($tip_opl == 1) {
								echo "Проживание: $st_kro.р<br>";
								echo "Оплата без взноса, осталось внести: "; $poln=$st_kro - $summa_opl + $sum_pit; echo"$poln.р";
									}else if ($tip_opl == 2) {
									echo "Взнос: $orgvznos.р<br>";
									echo "Оплата без проживания, осталось внести: "; $poln=$orgvznos - $summa_opl + $sum_pit; echo"$poln.р";
											}else if ($tip_opl == 3) {
												echo "<br>";
						...
						if ($tip_opl == 0){
							echo "Полная оплата, осталось внести: "; $poln=$summ_l - $summa_opl + $sum_pit; echo "$poln.р <br>";
							echo "Взнос: $orgvznos.р<br>";									
							echo "Проживание: $st_lux.р<br>";
							} else {if ($tip_opl == 1) {
								echo "Проживание: $st_lux.р<br>";
								echo "Оплата без взноса, осталось внести: "; $poln=$st_lux - $summa_opl + $sum_pit; echo"$poln.р";
									}else if ($tip_opl == 2) {
										echo "Взнос: $orgvznos.р<br>";
										echo "Оплата без проживания, осталось внести: "; $poln=$orgvznos - $summa_opl + $sum_pit; echo"$poln.р";
											}else if ($tip_opl == 3) {
						...
						if ($tip_opl == 0){
							...
							echo "Полная оплата, осталось внести: "; $poln=$summ_s - $summa_opl + $sum_pit; echo "$poln.р";
							} else {if ($tip_opl == 1) {
							echo "Проживание: $st_spa.р<br>";
							echo "Оплата без взноса, осталось внести: "; $poln=$st_spa - $summa_opl + $sum_pit; echo"$poln.р";
									}else if ($tip_opl == 2) {
									echo "Взнос: $orgvznos.р<br>";
									echo "Оплата без проживания, осталось внести: "; $poln=$orgvznos - $summa_opl + $sum_pit; echo"$poln.р";
											}else if ($tip_opl == 3) {
					...
					//поменять сумму оплаты
							if ($id_user=="539" or $id_iser=="1854") {
					...
					//указать произвольную оплату
							if ($id_user=="539" or $id_iser=="1854") {
							echo "<div style=\"border:1px solid black; padding:5px;\">
							Кастомная сумма оплаты:
							<form method=post action=changeopl.php?id=$id>
							<input style=\"width:60px;\" type=text name=new_summa_opl value = $new_summa_opl>";
							
							$ok_page = mysql_query("select * from room_us where  id_us=$id_u and god = $god_site");
								while ($t_page = mysql_fetch_array($ok_page))
							   {
								  $new_summa_opl = $t_page['tip_opl'];
					...
					//конец указать произвольную оплату
				}
            }
        }
...
<tr><td>Город: </td><td><a href = \"$site/users.php?g=$gorod_us\">$gorod_us</a></td></tr>";
		...
		echo "<h3><a href=\"#\">Питание:</a></h3><div>"; opros2(); 
		echo"</div>";				
		}
        if ($id_u == $id_user or $adm == 1 or $master == $id_user or $id_user=="539" or $id_user=="1854") {
            echo "
<p>
<div align=\"right\">
...
<span class=\"ncc\"><a href=\"$site/profile/del.php?id=$id\"  onclick=\"return confirm('Удалить? Уверены?')\">Удалить пользователя</a></span> 
...
<li style=\"margin:0px 3px 0px 0px;\">
<b class=\"nc\"><b class=\"nc1\"><b></b></b><b class=\"nc2\"><b></b></b></b>
<span class=\"ncc\"><a href=\"profile/ban.php?id=$id_u\" onclick=\"return confirm('Отправить в бан? Точно?')\">Заблокировать пользователя</a></span> 
<b class=\"nc\"><b class=\"nc2\"><b></b></b><b class=\"nc1\"><b></b></b></b>
</li>
</ul>
</div>";
        }
				if ($id_user == "539" or $id_user=="1854") {
